...
!(function (global) {
  ...
  global.getIP = function (ip) {
    global.returnCitySN = {}
    global.returnCitySN.cip = ip
  ...
  var trackEvent = (global.trackEvent = {
    ...
    getIpData: function (trackInitInfo) {
      ...
      if (!ipData || (!!ipData && now - JSON.parse(ipData).time > 15 * 24 * 60 * 60 * 1000) || (!!ipData && !JSON.parse(ipData).cip)) {
        trackEvent.ajax({
          url: '//bsmnserv.yunzhangfang.com/publicip.json',
          method: 'get',
          success: function(res) {
            if(res.code == 200) {
              getIP(res.data.split(',')[0] || '')
